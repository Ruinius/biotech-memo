# Biotech-Memo Framework

An automated, operator-grade framework designed to research complex biotechnology and artificial intelligence domains—specifically Forward Deployed Engineering (FDE) and generative AI systems—and compile premium, investor-ready executive strategy memos. 
...
*   **Custom Research Custom Skill (`research-memo`)**: A structured, step-by-step framework located at `skills/research_memo/SKILL.md` to guide agents or developers through topic definition, multi-source literature search, outline approval, and executive drafting.
...
*   **Structured Research Database**: Dedicated storage in `research/` for source-level findings, grounding all strategic assertions and market metrics in hard reality.
...
├── reference/           # Master structural reference memos (e.g., FDE Evolution reference)
...
Define the target research topics or organizational dynamics (e.g., AI-native biotech setups, Chinese asset licensing, reducing trial bias).

### Step 3: Scan Primary Source Inputs
Place highly trusted manuals, interviews, or transcripts inside `inputs/`. Extract key insights to ground all downstream arguments.

### Step 4: Multi-Source Web & Science Research
Execute searches to gather relevant data. For each major source, create a dedicated file under `research/` (e.g., `research/ai_native_biotechs.md`) cataloging:
