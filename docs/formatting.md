...
This document establishes the strict markdown formatting, typography, and layout rules for enterprise AI strategy memos, derived from [20260520_FDE_Evolution_Memo_vSHARED.md](file:///f:/AIML%20projects/biotech-memo/reference/20260520_FDE_Evolution_Memo_vSHARED.md). 
...
* **Level 3 Headings (`###`)**: Used for subsections, case studies, specific role descriptions, or strategic imperatives.
  * Example: `### Palantir: The Ontology-Driven Blueprint`
...
* **First Column Style**: Bold the category names in the leftmost column to act as clean row labels (e.g., `**FDE Nomenclature**`).
...
* **Term Highlighting (Bolding)**: Bold key operational roles, custom platforms, and specialized terms on their first appearance in a section (e.g., `**Deltas**`, `**Forward Deployed Data Scientist (FDDS)**`).
...
2. **No Developer Alert Blocks / GitHub Callouts**: Absolutely do not use GitHub-style alert wrappers (such as `[!IMPORTANT]`, `[!NOTE]`, `[!WARNING]`, `[!TIP]`). Use standard, standard blockquotes (`>`) instead.
